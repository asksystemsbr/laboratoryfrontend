...
export default function GrupoUsuarioList() {
  ...
  const [filteredItems, setFilteredItems] = useState<GrupoUsuario[]>([]);
  ...
  const loadGrupoUsuarios = useCallback(async () => {
    try {
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="container mx-auto p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Grupos de Usuários</h1>
          <button
        ...
        <table className="min-w-full bg-white border border-gray-300 rounded-lg">
          ...
          <tbody>
            {currentItems.map((grupo) => (
              <tr key={grupo.id} className="border-t border-gray-300 hover:bg-gray-100 transition">
                <td className="py-3 px-6 text-left text-sm text-gray-800">{grupo.descricao}</td>
                    ...
                    <div className="absolute mt-2 w-32 bg-white border rounded-lg shadow-lg z-10">
                      <ul className="py-1">
                        ...
                        <li
                          ...
                          onClick={() => openDeleteConfirm(grupo.id!)}
                        >
                          Excluir
                        </li>
                      </ul>
        ...
        >
          {isEditing ? (
            <GrupoUsuarioEditForm
              grupoUsuario={editingGrupo!}
              onSave={handleSave}
              onClose={() => setModalIsOpen(false)}
              setSnackbar={setSnackbar}
            />
          ) : (
            <GrupoUsuarioCreateForm
              onSave={handleSave}
              onClose={() => setModalIsOpen(false)}
              setSnackbar={setSnackbar}
            />
        ...
        <ConfirmationModal
          ...
          message="Tem certeza de que deseja excluir este grupo? Esta ação não pode ser desfeita."
