...
export default function ItemsList() {
  ...
  const [filtered, setFiltered] = useState<Recepcao[]>([]);
    ...
    <div className="flex h-screen">
      ...
      <div className="container mx-auto p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Solcitantes</h1>
          <button
          ...
          <tbody>
            {filtered.map((item) => (
              <tr key={item.id} className="border-t border-gray-200">
                <td className="py-2 px-4 text-left">{item.nomeRecepcao}</td>
        ...
        >
          {isEditing ? (
            <RecepcaoEditForm recepcao={editingItem!} onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          ) : (
            <RecepcaoCreateForm onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          )}
        </Modal>
