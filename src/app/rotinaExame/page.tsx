...
export default function ItemsList() {
  const [items, setItems] = useState<RotinaExame[]>([]);
  const [filtered, setFiltered] = useState<RotinaExame[]>([]);
  const [searchTerm, setSearchTerm] = useState(''); // Estado para o termo de busca
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [editingItem, setEditingItem] = useState<RotinaExame | null>(null);
  ...
  const handleDelete = async () => {
    if (itemToDelete !== null) {
      try {
        await axios.delete(`/api/RotinaExame/${itemToDelete}`);
  ...
  const handleEdit = (item: RotinaExame) => {
    setEditingItem(item);
    setIsEditing(true);
    setModalIsOpen(true);
  };
    ...
    <div className="flex h-screen">
      ...
      <div className="container mx-auto p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Rotina de Exame</h1>
          <button
        ...
        >
          {isEditing ? (
            <RotinaExameEditForm rotinaExame={editingItem!} onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          ) : (
            <RotinaExameCreateForm onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          )}
        </Modal>
