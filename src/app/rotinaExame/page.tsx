...
import { RotinaExameCreateForm } from './rotinaExamecreate';
import { RotinaExameEditForm } from './rotinaExameedit';
...
export default function RotinaExameList() {
  const [, setRotinas] = useState<RotinaExame[]>([]);
  const [filteredRotinas, setFilteredRotinas] = useState<RotinaExame[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [editingRotina, setEditingRotina] = useState<RotinaExame | null>(null);
  const [snackbar, setSnackbar] = useState(new SnackbarState());
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  const [rotinaToDelete, setRotinaToDelete] = useState<number | null>(null);
  ...
  const loadRotinas = useCallback(async () => {
    try {
      const response = await axios.get('/api/RotinaExame');
      setRotinas(response.data);
      setFilteredRotinas(response.data);
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    } catch (error) {
      setSnackbar(new SnackbarState('Erro ao carregar rotinas de exame!', 'error', true));
    }
  }, []);
  ...
  // Função para editar rotina de exame
  const handleEdit = (rotina: RotinaExame) => {
  ...
  const handleSave = () => {
    ...
    setSnackbar(new SnackbarState('Rotina salva com sucesso!', 'success', true));
    loadRotinas();
  };
  ...
  const handleDelete = async () => {
    if (rotinaToDelete !== null) {
      try {
        await axios.delete(`/api/RotinaExame/${rotinaToDelete}`);
        setSnackbar(new SnackbarState('Rotina excluída com sucesso!', 'success', true));
        loadRotinas();
        setDeleteConfirmOpen(false);
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      } catch (error) {
        setSnackbar(new SnackbarState('Erro ao excluir rotina de exame!', 'error', true));
      }
    }
  };

  const openDeleteConfirm = (id: number) => {
    setRotinaToDelete(id);
  ...
  const currentRotinas = filteredRotinas.slice(indexOfFirstRecord, indexOfLastRecord);
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="container mx-auto p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Rotinas de Exames</h1>
          <button
            onClick={handleNew}
        ...
        <table className="min-w-full bg-white border border-gray-300 rounded-lg">
          ...
          <tbody>
            {currentRotinas.map((rotina) => (
              <tr key={rotina.id} className="border-t border-gray-300 hover:bg-gray-100 transition">
                <td className="py-3 px-6 text-left text-sm text-gray-800">{rotina.descricao}</td>
                <td className="py-3 px-6 text-left relative dropdown-actions">
                  <button
                    onClick={() => toggleDropdown(rotina.id!)}
                    ...
                    <div className="absolute mt-2 w-32 bg-white border rounded-lg shadow-lg z-10">
                      <ul className="py-1">
                        ...
                        <li
                          ...
                          onClick={() => openDeleteConfirm(rotina.id!)}
                        >
                          Excluir
                        </li>
                      </ul>
        ...
        <div className="flex justify-between mt-4">
          ...
          <button
            onClick={() => paginate(currentPage + 1)}
            disabled={currentPage === Math.ceil(filteredRotinas.length / recordsPerPage)}
            className="bg-gray-200 hover:bg-gray-300 text-gray-600 font-semibold py-2 px-4 rounded-lg flex items-center justify-center shadow-sm transition-all duration-200"
          >
            Próxima
            <svg
        ...
        >
          {isEditing ? (
            <RotinaExameEditForm
              rotinaExame={editingRotina!}
              onSave={handleSave}
              onClose={() => setModalIsOpen(false)}
              setSnackbar={setSnackbar}
            />
          ) : (
            <RotinaExameCreateForm
              onSave={handleSave}
              onClose={() => setModalIsOpen(false)}
              setSnackbar={setSnackbar}
            />
        ...
        <ConfirmationModal
          ...
          message="Tem certeza de que deseja excluir esta rotina de exame? Esta ação não pode ser desfeita."
