...
interface FormaPagamentoCreateFormProps {
  onSave: () => void;
  onClose: () => void;
  setSnackbar: (state: SnackbarState) => void; // Adiciona o setSnackbar como prop
}

export const FormaPagamentoCreateForm = ({ onSave, onClose,setSnackbar  }: FormaPagamentoCreateFormProps) => {
  ...
  const onSubmit = async (data: FormaPagamento) => {
    ...
    try {
        ...
        await axios.post('/api/FormaPagamento', data);
        reset();
        onSave();
      } catch (error) {
        console.log(error);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-4">
      <h2 className="text-xl font-bold mb-4">Nova Forma Pagamento</h2>
      <div className="mb-4">
