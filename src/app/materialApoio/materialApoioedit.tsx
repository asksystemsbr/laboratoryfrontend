...
interface MaterialApoioEditFormProps  {
...
export const MaterialApoioEditForm  = ({ materialApoio, onSave, onClose,setSnackbar  }: MaterialApoioEditFormProps ) => {
  const { register, handleSubmit, reset,formState: { errors } } = useForm<MaterialApoio>({
    defaultValues: materialApoio,
  });

  const onSubmit = async (data: MaterialApoio) => {
    try {
        await axios.put(`/api/MaterialApoio/${materialApoio.id}`, data);
        reset();
        onSave();
      } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-4">
      <h2 className="text-xl font-bold mb-4">Editar Material de Apoio</h2>
      <div className="mb-4">
