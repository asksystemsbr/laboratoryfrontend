//src/app/materialApoio/page.tsx
"use client";
...
export default function ItemsList() {
  ...
  const [filtered, setFiltered] = useState<MaterialApoio[]>([]);
    ...
    <div className="flex h-screen">
      ...
      <div className="container mx-auto p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Material de Apoio</h1>
          <button
        ...
        <table className="min-w-full bg-white border border-gray-200">
          <thead>
            <tr>
              <th className="py-2 px-4 text-left">Nome do Material</th>
          ...
          <tbody>
            {filtered.map((item) => (
              <tr key={item.id} className="border-t border-gray-200">
                <td className="py-2 px-4 text-left">{item.nomeMaterial}</td>
        ...
        >
          {isEditing ? (
            <MaterialApoioEditForm  materialApoio={editingItem!} onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          ) : (
            <MaterialApoioCreateForm  onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          )}
        </Modal>
