...
export default function ItemsList() {
  ...
  const handleDelete = async () => {
    if (itemToDelete !== null) {
      try {
        await axios.delete(`/api/Setor/${itemToDelete}`);
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="container mx-auto p-8">
        ...
        >
          {isEditing ? (
            <SetorEditForm setor={editingItem!} onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          ) : (
            <SetorCreateForm onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          )}
        </Modal>
