...
export const ExameEditForm = ({ exame, onSave, onClose, setSnackbar }: ExameEditFormProps) => {
  ...
  useEffect(() => {
    ...
    Promise.all([loadEspecialidades(), loadSetores(), loadMaterialApoio()]).then(() => setIsLoaded(true));
  }, [setSnackbar]);

  useEffect(() => {
    if (isLoaded) {
      setValue('estabilidade', exame.estabilidade);
      setValue('setor_id', exame.setor_id);
      setValue('material_apoio_id', exame.material_apoio_id);
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-4">
      ...
      {/* PreparoC */}
      <div className="mb-4">
        <label className="block text-gray-700">Preparo Criança</label>
        <input
          {...register('preparoc')}
          className="border rounded w-full py-2 px-3 mt-1"
        />
