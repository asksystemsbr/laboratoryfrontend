...
import { ExameCreateForm } from './examecreate';
import { ExameEditForm } from './exameedit';
...
export default function ExameList() {
  ...
  const [filtered, setFiltered] = useState<Exame[]>([]);
  ...
  const handleDelete = async () => {
    if (itemToDelete !== null) {
      try {
        await axios.delete(`/api/Exame/${itemToDelete}`);
        setSnackbar(new SnackbarState('Exame excluído com sucesso!', 'success', true));
        loadItems();
        setDeleteConfirmOpen(false);
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      } catch (error) {
        setSnackbar(new SnackbarState('Erro ao excluir exame!', 'error', true));
      }
    }
  };
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="container mx-auto p-8">
        ...
        <table className="min-w-full bg-white border border-gray-300 rounded-lg">
          <thead>
            <tr className="bg-gray-50">
              <th className="py-3 px-6 text-left text-sm font-semibold text-gray-600 border-b">Código do Exame</th>
              <th className="py-3 px-6 text-left text-sm font-semibold text-gray-600 border-b">Nome do Exame</th>
              <th className="py-3 px-6 text-left text-sm font-semibold text-gray-600 border-b">Sinônimos</th>
          ...
          <tbody>
            {currentItems.map((item) => (
              <tr key={item.id} className="border-t border-gray-300 hover:bg-gray-100 transition">
                <td className="py-3 px-6 text-left text-sm text-gray-800">{item.codigoExame}</td>
                <td className="py-3 px-6 text-left text-sm text-gray-800">{item.nomeExame}</td>
                <td className="py-3 px-6 text-left text-sm text-gray-800">{item.sinonimos}</td>
        ...
        <ConfirmationModal
          ...
          message="Tem certeza de que deseja excluir este exame? Esta ação não pode ser desfeita."
