...
export const EmpresaEditForm = ({ empresa, onSave, onClose, setSnackbar }: EmpresaEditFormProps) => {
  const { register, handleSubmit,  formState: { errors },setValue } = useForm<Empresa>({ defaultValues: empresa });
  ...
  useEffect(() => {
    ...
    Promise.all([fetchUF(),fetchEmpresaCategoria()]).then(() => setIsLoaded(true));
  }, [setSnackbar]);

  useEffect(() => {
    const fetchEndereco = async () => {
      try {
        const response = await axios.get(`/api/Endereco/${empresa.enderecoId}`);
  ...
  }, [isLoaded,empresa.enderecoId, setSnackbar]);

  useEffect(() => {
    if (isLoaded) {
      setValue('categoriaEmpresaId', empresa.categoriaEmpresaId);
    }
  }, [isLoaded, empresa, setValue]);
