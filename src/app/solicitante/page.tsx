...
export default function SolicitanteList() {
  const [items, setItems] = useState<Solicitante[]>([]);
  const [filteredItems, setFilteredItems] = useState<Solicitante[]>([]);
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="container mx-auto p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Solicitantes</h1>
          <button
        ...
        <table className="min-w-full bg-white border border-gray-300 rounded-lg">
          <thead>
            <tr className="bg-gray-50">
              ...
              <th className="py-3 px-6 text-left text-sm font-semibold text-gray-600 border-b">Telefone</th>
          ...
          <tbody>
            {currentItems.map((item) => (
              <tr key={item.id} className="border-t border-gray-300 hover:bg-gray-100 transition">
                ...
                <td className="py-3 px-6 text-left text-sm text-gray-800">{item.telefone}</td>
        ...
        >
          {isEditing ? (
            <SolicitanteEditForm
              solicitante={editingItem!}
              onSave={handleSave}
              onClose={() => setModalIsOpen(false)}
              setSnackbar={setSnackbar}
            />
          ) : (
            <SolicitanteCreateForm
              onSave={handleSave}
              onClose={() => setModalIsOpen(false)}
              setSnackbar={setSnackbar}
            />
