//src/app/exameApoio/page.tsx
"use client";
import React from 'react';
import { useState, useEffect } from 'react';
import { ExameApoioCreateForm } from './exameApoiocreate';
import { ExameApoioEditForm } from './exameApoioedit';
...
export default function ItemsList() {
  const [items, setItems] = useState<ExameApoio[]>([]);
  const [filtered, setFiltered] = useState<ExameApoio[]>([]);
  const [searchTerm, setSearchTerm] = useState(''); // Estado para o termo de busca
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [editingItem, setEditingItem] = useState<ExameApoio | null>(null);
  ...
  const handleDelete = async () => {
    if (itemToDelete !== null) {
      try {
        await axios.delete(`/api/ExameApoio/${itemToDelete}`);
  ...
  const handleEdit = (item: ExameApoio) => {
    setEditingItem(item);
    setIsEditing(true);
    setModalIsOpen(true);
  };
    ...
    <div className="flex h-screen">
      ...
      <div className="container mx-auto p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Exame de Apoio</h1>
          <button
        ...
        >
          {isEditing ? (
            <ExameApoioEditForm   exameApoio={editingItem!} onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          ) : (
            <ExameApoioCreateForm   onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          )}
        </Modal>
