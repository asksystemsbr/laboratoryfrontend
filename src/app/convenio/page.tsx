...
export default function ItemsList() {
  ...
  const handleDelete = async () => {
    if (itemToDelete !== null) {
      try {
        await axios.delete(`/api/Convenio/${itemToDelete}`);
  ...
  const handleEdit = (item: Convenio) => {
    setEditingItem(item);
    setIsEditing(true);
    setModalIsOpen(true);
  };
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="container mx-auto p-8">
        ...
        >
          {isEditing ? (
            <ConvenioEditForm convenio={editingItem!} onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          ) : (
            <ConvenioCreateForm onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          )}
        </Modal>
