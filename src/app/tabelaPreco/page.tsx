...
export default function TabelaPrecoList() {
  const [items, setItems] = useState<TabelaPreco[]>([]);
  const [filteredItems, setFilteredItems] = useState<TabelaPreco[]>([]);
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="container mx-auto p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Tabela de Preço</h1>
          <button
          ...
          <tbody>
            {currentItems.map((item) => (
              <tr key={item.id} className="border-t border-gray-300 hover:bg-gray-100 transition">
                <td className="py-3 px-6 text-left text-sm text-gray-800">{item.nome}</td>
        ...
        >
          {isEditing ? (
            <TabelaPrecEditForm
              tabelaPreco={editingItem!}
              onSave={handleSave}
              onClose={() => setModalIsOpen(false)}
              setSnackbar={setSnackbar}
            />
          ) : (
            <TabelaPrecoCreateForm
              onSave={handleSave}
              onClose={() => setModalIsOpen(false)}
              setSnackbar={setSnackbar}
            />
