//src/app/recipienteAmostra/page.tsx
"use client";
...
export default function ItemsList() {
  const [items, setItems] = useState<RecipienteAmostra[]>([]);
  const [filtered, setFiltered] = useState<RecipienteAmostra[]>([]);
  const [searchTerm, setSearchTerm] = useState(''); // Estado para o termo de busca
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [editingItem, setEditingItem] = useState<RecipienteAmostra | null>(null);
  ...
  const handleDelete = async () => {
    if (itemToDelete !== null) {
      try {
        await axios.delete(`/api/RecipienteAmostra/${itemToDelete}`);
  ...
  const handleEdit = (item: RecipienteAmostra) => {
    setEditingItem(item);
    setIsEditing(true);
    setModalIsOpen(true);
  };
    ...
    <div className="flex h-screen">
      ...
      <div className="container mx-auto p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Recipiente Amostra</h1>
          <button
            onClick={handleCreate}
            className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
          >
            Nova Amostra Recipiente
          </button>
        </div>
        ...
        >
          {isEditing ? (
            <RecipienteAmostraEditForm recipienteAmostra={editingItem!} onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          ) : (
            <RecipienteAmostraCreateForm onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          )}
        </Modal>
