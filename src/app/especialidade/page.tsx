//src/app/especialidade/page.tsx
"use client";
import { useState, useEffect } from 'react';
import { EspecialidadeCreateForm } from './especialidadecreate';
import { EspecialidadeEditForm } from './especialidadeedit';
...
export default function ItemsList() {
  ...
  const handleDelete = async () => {
    if (itemToDelete !== null) {
      try {
        await axios.delete(`/api/Especialidade/${itemToDelete}`);
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="container mx-auto p-8">
        ...
        >
          {isEditing ? (
            <EspecialidadeEditForm especialidade={editingItem!} onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          ) : (
            <EspecialidadeCreateForm onSave={handleSave} onClose={() => setModalIsOpen(false)} setSnackbar={setSnackbar} />
          )}
        </Modal>
