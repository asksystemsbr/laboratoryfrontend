...
export const ClienteEditForm = ({ clienteId, onSave, onClose, setSnackbar }: ClienteEditFormProps) => {
  ...
  const situacaoId = watch('situacaoId'); // Verifica o valor selecionado no campo de Situação
  ...
  const loadCliente = useCallback(async () => {
    try {
      ...
      if (data.nascimento) {
        const validNascimentoDate = new Date(data.nascimento);
        if (!isNaN(validNascimentoDate.getTime())) {
          const formattedNascimentoDate = validNascimentoDate.toISOString().split('T')[0];
          setValue('nascimento', formattedNascimentoDate);
        }
      }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50">
      <form onSubmit={handleSubmit(onSubmit)} className="p-4 max-w-4xl w-full bg-white rounded-lg shadow-lg overflow-y-auto max-h-screen">
        <h2 className="text-xl font-bold mb-2 text-gray-800">Editar Cliente</h2>

        <div className="flex space-x-2 mb-3">
          <div className="w-1/2">
            <label className="block text-gray-800">Nome</label>
            <input {...register('nome', { required: 'O nome é obrigatório' })} className="border rounded w-full py-1 px-3 mt-1 text-gray-800" />
        ...
        <div className="flex space-x-2 mb-3">
          ...
          <div className="w-1/4">
            ...
            <select {...register('planoId')} className="border rounded w-full py-1 px-3 mt-1 text-gray-800">
              <option value="" disabled>Selecione</option> {/* Ajustado para estar no topo */}
              {planos.map((plano) => (
          ...
          <div className="w-1/4">
            ...
            <select
                ...
                className={`border rounded w-full py-1 px-3 mt-1 text-gray-800 ${
                  situacaoId === 1 ? 'bg-green-200' : situacaoId === 0 ? 'bg-red-200' : ''
                }`}
                >
                <option value={1}>Ativo</option>
                <option value={0}>Inativo</option>
            </select>
            {errors.situacaoId && <p className="text-red-500 text-sm">{errors.situacaoId?.message}</p>}
          </div>
        </div>
        ...
        <div className="flex space-x-2 items-end mb-3">
          <div className="w-1/2">
            ...
            <div className="flex items-center">
              <InputMask {...register('cpfCnpj', { required: 'CPF/CNPJ é obrigatório' })} mask={isCNPJ ? '99.999.999/9999-99' : '999.999.999-99'} className="border rounded w-full py-1 px-3 mt-1 text-gray-800" />
              <button type="button" onClick={toggleMask} className="ml-2 py-1 px-3 bg-blue-500 text-white rounded">
          ...
          <div className="w-1/4">
            ...
            <InputMask {...register('rg')} mask="99.999.999-9" className="border rounded w-full py-1 px-3 mt-1 text-gray-800" />
            {errors.rg && <p className="text-red-500 text-sm">{errors.rg?.message}</p>}
          </div>

          <div className="w-1/4">
            <label className="block text-gray-800">CEP</label>
            <InputMask value={cep} mask="99999-999" className="border rounded w-full py-1 px-3 mt-1 text-gray-800" onChange={handleCepChange} />
          </div>

          <div className="w-1/4">
            <label className="block text-gray-800">Data de Nascimento</label>
            <input type="date" {...register('nascimento')} className="border rounded w-full py-1 px-3 mt-1 text-gray-800" />
          </div>
        </div>
        ...
        {isCNPJ && (
          <div className="flex space-x-2 mb-3">
            <div className="w-1/3">
              ...
              <input {...register('nomeFantasia')} className="border rounded w-full py-1 px-3 mt-1 text-gray-800" />
            </div>

            <div className="w-1/3">
              <label className="block text-gray-800">Inscrição Estadual (IE)</label>
              <input {...register('ie')} className="border rounded w-full py-1 px-3 mt-1 text-gray-800" />
            </div>

            <div className="w-1/3">
              <label className="block text-gray-800">Inscrição Municipal (IM)</label>
              <input {...register('im')} className="border rounded w-full py-1 px-3 mt-1 text-gray-800" />
            </div>
          </div>
        )}
        ...
        {isMenorDeIdade && (
          <div className="flex space-x-2 mb-3">
            <div className="w-1/3">
              ...
              <input {...register('nomeResponsavel')} className="border rounded w-full py-1 px-3 mt-1 text-gray-800" />
            </div>

            <div className="w-1/3">
              <label className="block text-gray-800">CPF do Responsável</label>
              <InputMask {...register('cpfResponsavel')} mask="999.999.999-99" className="border rounded w-full py-1 px-3 mt-1 text-gray-800" />
            </div>

            <div className="w-1/3">
              <label className="block text-gray-800">Telefone do Responsável</label>
              <InputMask {...register('telefoneResponsavel')} mask="(99) 99999-9999" className="border rounded w-full py-1 px-3 mt-1 text-gray-800" />
            </div>
          </div>
        )}
        ...
        <div className="flex space-x-2 mb-3">
          <div className="w-3/4">
            ...
            <input value={logradouro} onChange={(e) => setLogradouro(e.target.value)} className="border rounded w-full py-1 px-3 mt-1 text-gray-800 bg-gray-200" />
          </div>

          <div className="w-1/4">
            <label className="block text-gray-800">Número</label>
            <input value={numero} onChange={(e) => setNumero(e.target.value)} className="border rounded w-full py-1 px-3 mt-1 text-gray-800 bg-gray-200" />
          </div>
        </div>
        ...
        <div className="flex space-x-2 mb-3">
          <div className="w-1/3">
            ...
            <input value={complemento} onChange={(e) => setComplemento(e.target.value)} className="border rounded w-full py-1 px-3 mt-1 text-gray-800 bg-gray-200" />
          </div>

          <div className="w-1/3">
            <label className="block text-gray-800">Bairro</label>
            <input value={bairro} onChange={(e) => setBairro(e.target.value)} className="border rounded w-full py-1 px-3 mt-1 text-gray-800 bg-gray-200" />
          </div>

          <div className="w-1/3">
            <label className="block text-gray-800">Cidade</label>
            <input value={localidade} onChange={(e) => setLocalidade(e.target.value)} className="border rounded w-full py-1 px-3 mt-1 text-gray-800 bg-gray-200" />
          </div>

          <div className="w-1/6">
            <label className="block text-gray-800">UF</label>
            <input value={uf} onChange={(e) => setUf(e.target.value)} className="border rounded w-full py-1 px-3 mt-1 text-gray-800 bg-gray-200" />
          </div>
        </div>

        <div className="flex justify-end">
          <button type="button" onClick={onClose} className="mr-2 py-2 px-4 rounded bg-gray-500 text-white">Cancelar</button>
          <button type="submit" className="py-2 px-4 rounded bg-blue-500 text-white">Salvar</button>
        </div>
      </form>
    </div>
  );
